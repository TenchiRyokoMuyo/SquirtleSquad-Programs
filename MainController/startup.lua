...
local function safeSerializeToFile(path, value) ensureDir(); local h=fs.open(path,"w"); if not h then return false end; h.write(textutils.serialize(value)); h.close(); return true end
...
local function saveState() safeSerializeToFile(STATE_FILE,state) end
...
local function attachMonitor()
  monitor=nil
  for _,side in ipairs(peripheral.getNames()) do
    if peripheral.getType(side)=="monitor" then monitor=peripheral.wrap(side); pcall(function() monitor.setTextScale(0.5) end); return true end
  end
  return false
end
...
local function header(t)
  if t.isColor and t.isColor() then t.setBackgroundColor(colors.black); t.setTextColor(colors.cyan) end
...
local function promptCoord(name)
  ...
  local x=promptNumber("X") local y=promptNumber("Y") local z=promptNumber("Z")
  if not x or not y or not z then print("Invalid coordinates."); sleep(1); return nil end
  return {x=x,y=y,z=z}
end
local function makeId(prefix) return prefix.."-"..tostring(os.epoch("utc")).."-"..tostring(math.random(1000,9999)) end
local function cloneCoord(c) if not c then return nil end return {x=c.x,y=c.y,z=c.z} end
local function cloneBounds(b) if not b then return nil end return {minX=b.minX,maxX=b.maxX,minY=b.minY,maxY=b.maxY,minZ=b.minZ,maxZ=b.maxZ} end

local function chooseMenu(title, items, subtitle)
  local selected, top = 1, 1
  while true do
    term.redirect(terminal); header(term)
    print("")
    print(title)
    ...
    for row=0,visible-1 do
      ...
      if i==selected then bcolor(colors.blue); color(colors.white); print("> "..items[i][1]); bcolor(colors.black)
      else color(colors.white); print("  "..items[i][1]) end
    end
    color(colors.white)
    ...
    elseif k==keys.enter then return items[selected][2], selected
    elseif k==keys.backspace or k==keys.left then return nil,nil end
  end
end
...
local function drawDashboard()
  ...
  header(monitor)
  local w,h=monitor.getSize(); local y=4
  local miners,foremen,gpshosts=0,0,0
  for _,a in pairs(state.agents) do
    if a.role=="miner" then miners=miners+1 elseif a.role=="foreman" then foremen=foremen+1 elseif a.role=="gps" then gpshosts=gpshosts+1 end
  end
  ...
  monitor.setCursorPos(1,y); monitor.write("Active Job: "..tostring(state.activeJobId or "none")); y=y+2
  monitor.setCursorPos(1,y); monitor.write("Recent Logs:"); y=y+1
  local start=math.max(1,#state.logs-(h-y)+1)
  for i=start,#state.logs do if y>h then break end; monitor.setCursorPos(1,y); monitor.write(string.sub(state.logs[i],1,w)); y=y+1 end
  term.redirect(old)
end
...
local function createJob()
  ...
  if shape=="rect" then
    ...
    job.a=promptCoord("corner A"); job.b=promptCoord("corner B")
  ...
  elseif shape=="tunnel" then
    ...
    job.width=promptNumber("Width",3); job.height=promptNumber("Height",job.width)
  ...
  saveState(); print("Job created: "..job.id); sleep(1)
end
...
local function activeAgents(role)
  ...
  for id,a in pairs(state.agents) do
    if (not role or a.role==role) and now-(a.lastSeen or 0)<45000 then table.insert(t,a) end
  end
  table.sort(t,function(a,b) return tostring(a.id)<tostring(b.id) end)
  return t
end
local function activeMiners() return activeAgents("miner") end
local function activeForemen() return activeAgents("foreman") end
...
local function computeJobBounds(job)
  ...
  elseif job.shape=="cylinder" or job.shape=="cone" then
    ...
    set(job.origin.x+job.radius, job.origin.y+jobLayers(job)-1, job.origin.z+job.radius)
...
local function buildTeams()
  local miners=activeMiners(); local foremen=activeForemen(); state.teams={}
  if #miners==0 then return end
  for i,miner in ipairs(miners) do
    local f=nil
    if #foremen>0 then f=foremen[((i-1)%#foremen)+1] end
    table.insert(state.teams,{id="team-"..tostring(i),minerId=miner.id,minerNet=miner.netId,foremanId=f and f.id or nil,foremanNet=f and f.netId or nil,groupIndex=i,groupCount=#miners})
  end
  saveState()
end

local function compactJob(job)
  return {id=job.id,shape=job.shape,status=job.status,origin=cloneCoord(job.origin),a=cloneCoord(job.a),b=cloneCoord(job.b),radius=job.radius,layers=jobLayers(job),height=jobLayers(job),width=job.width,torchSpacing=job.torchSpacing,shallow=job.shallow,created=job.created}
end
local function compactSector(sector) return {id=sector.id,index=sector.index,count=sector.count,bounds=cloneBounds(sector.bounds),fullBounds=cloneBounds(sector.fullBounds)} end
local function safeSend(target,packet,protocol)
  if not target then return false end
  local ok,payload=pcall(textutils.serialize,packet)
  if not ok then print("Packet serialization failed:"); print(tostring(payload)); log("Packet serialization failed: "..tostring(payload)); return false end
...
local function assignActiveJob()
  ...
  if #foremen==0 then log("Assigning job with 0 foremen present; miners are allowed to run without foremen.") end
  ...
  for i,team in ipairs(state.teams) do
    ...
    local sector={id="sector-"..i,index=i,count=total,bounds={minX=sx1,maxX=sx2,minY=bounds.minY,maxY=bounds.maxY,minZ=bounds.minZ,maxZ=bounds.maxZ},fullBounds=cloneBounds(bounds)}
    team.jobId=job.id; team.sectorId=sector.id; team.sectorBounds=cloneBounds(sector.bounds); team.fullBounds=cloneBounds(sector.fullBounds); team.deployOrder=i; team.status="ASSIGNED"
    table.insert(state.deploymentQueue,team.id)
    if team.foremanNet then safeSend(team.foremanNet,{type="ASSIGN_FOREMAN",project=PROJECT,version=VERSION,teamId=team.id,minerId=team.minerId,minerNet=team.minerNet,jobId=job.id,job=jobPacket,sector=compactSector(sector),deployOrder=i,deployHold=true},PROTOCOL) end
    safeSend(team.minerNet,{type="ASSIGN_JOB",project=PROJECT,version=VERSION,teamId=team.id,minerId=team.minerId,foremanId=team.foremanId,foremanNet=team.foremanNet,jobId=job.id,job=jobPacket,sector=compactSector(sector),deployOrder=i,deployHold=true},PROTOCOL)
  end
  job.status="ASSIGNED"; saveState(); log("Assigned job to "..tostring(#state.teams).." miner teams.")
  if #state.deploymentQueue>0 then
    ...
    for _,team in ipairs(state.teams) do
      if team.id==first then
        ...
        if team.foremanNet then safeSend(team.foremanNet,{type="DEPLOY_NOW",teamId=team.id,jobId=job.id},PROTOCOL) end
...
local function pauseJob() for _,a in pairs(state.agents) do if a.netId then rednet.send(a.netId,{type="PAUSE_JOB"},PROTOCOL) end end; log("Pause sent.") end
local function resumeJob() for _,a in pairs(state.agents) do if a.netId then rednet.send(a.netId,{type="RESUME_JOB"},PROTOCOL) end end; log("Resume sent.") end
local function cancelJob()
  for _,a in pairs(state.agents) do if a.netId then rednet.send(a.netId,{type="CANCEL_JOB"},PROTOCOL) end end
  if state.activeJobId and state.jobs[state.activeJobId] then state.jobs[state.activeJobId].status="CANCELLED" end
  state.activeJobId=nil; saveState(); log("Cancel sent. Miners will return home if they have a home set.")
end

local function distanceFromController(a)
  if not (state.gps.x and state.gps.y and state.gps.z and a.x and a.y and a.z) then return "?" end
...
local function chooseAgent(title, roleFilter)
  ...
  if #agents==0 then header(term); print("No active agents found."); sleep(1.5); return nil end
  local items={}
  for _,a in ipairs(agents) do table.insert(items,{agentLabel(a),a}) end
...
local function resetConnectedSubhosts()
  local count=0
  for _,a in pairs(state.agents) do
    if a.role=="gps" and a.netId then rednet.send(a.netId,{type="RESET_GPS_COORDS"},PROTOCOL); count=count+1 end
  end
...
local function configureGPS()
  ...
  if c then state.gps.x,state.gps.y,state.gps.z=c.x,c.y,c.z; state.gps.hostEnabled=true; saveState(); log("Controller GPS host coords updated.") end
end
local function gpsDiagnostics()
  ...
  for _,a in pairs(state.agents) do
    if a.role=="gps" then print(" - "..tostring(a.label or a.id).." id "..tostring(a.netId).." at "..tostring(a.x)..","..tostring(a.y)..","..tostring(a.z).." last "..tostring(math.floor((os.epoch("utc")-(a.lastSeen or 0))/1000)).."s ago") end
  end
  print("") print("Press any key."); os.pullEvent("key")
end
local function viewLogs()
  header(term); print("Logs:")
...
local function clearHistory()
  state.logs={}
  for id,j in pairs(state.jobs) do if j.status=="COMPLETE" or j.status=="CANCELLED" then state.jobs[id]=nil end end
  saveState(); log("History cleared.")
end
...
local function nextDeployment(teamId)
  ...
  if state.deploymentQueue[1]==teamId then
    table.remove(state.deploymentQueue,1); saveState()
    ...
    if nextTeamId then
      for _,team in ipairs(state.teams) do
        if team.id==nextTeamId then
          ...
          log("Deployment started: "..team.id); return
        end
      end
    else log("All teams deployed.") end
  end
end
  ...
  while running do
    ...
    if type(msg)=="table" then
      ...
      if msg.type=="REGISTER" then
        ...
        a.id=id; a.netId=sender; a.role=msg.role; a.label=msg.label; a.status=msg.status or a.status; a.x=msg.x or a.x; a.y=msg.y or a.y; a.z=msg.z or a.z; a.lastSeen=now
      ...
      elseif msg.type=="HEARTBEAT" then
        ...
        local a=state.agents[id]
        a.netId=sender; a.role=msg.role or a.role; a.label=msg.label or a.label; a.status=msg.status; a.lastSeen=now; a.x=msg.x or a.x; a.y=msg.y or a.y; a.z=msg.z or a.z; a.facing=msg.facing or a.facing
      elseif msg.type=="REQUEST_ASSIGNMENT" then
        if state.activeJobId then log("Assignment recovery requested by "..sender); assignActiveJob() end
      elseif msg.type=="TEAM_READY_AT_ORIGIN" then log("Team at origin: "..tostring(msg.teamId)); nextDeployment(msg.teamId)
      elseif msg.type=="SECTOR_COMPLETE" then log("Sector complete: "..tostring(msg.teamId))
      elseif msg.type=="ERROR" then log("ERROR from "..sender..": "..tostring(msg.message)) end
    end
  end
end
local function dashboardLoop() while running do drawDashboard(); sleep(2) end end
local function controllerGpsHostLoop()
  while running do
    if state.gps and state.gps.hostEnabled and state.gps.x and state.gps.y and state.gps.z then
      ...
      pcall(function() shell.run("gps","host",tostring(state.gps.x),tostring(state.gps.y),tostring(state.gps.z)) end)
      term.redirect(prior)
    else sleep(5) end
    sleep(1)
  end
...
ensureDir(); state=healState(safeLoad(STATE_FILE) or state); saveState(); openModem(); attachMonitor(); math.randomseed(os.epoch("utc")%100000)
...
term.redirect(terminal); term.clear(); term.setCursorPos(1,1); print(PROJECT.." "..VERSION.." stopped.")
