...
local function trySerialize(label, value)
  ...
  if not ok then
    ...
    pcall(function()
      rawLog("SERIALIZE FAILURE: " .. tostring(label))
      rawLog(tostring(result))
    end)
...
local function healState(s)
  ...
  if s.gps.hostEnabled == nil then
    s.gps.hostEnabled = true
  end
...
local function saveState()
  return safeSerializeToFile(STATE_FILE, state)
end
...
local function safeSend(target, packet, protocol, label)
  if not target then
  ...
  local serialized = trySerialize("rednet:" .. tostring(label), packet)
  if not serialized then
    print("FAILED TO SEND PACKET")
    print("LABEL: " .. tostring(label))
    return false
  ...
  local rebuilt = textutils.unserialize(serialized)
  return rednet.send(target, rebuilt, protocol)
end
...
local function writeCentered(t, y, text, col)
  ...
  if col and t.isColor and t.isColor() then
    t.setTextColor(col)
  end
...
local function drawDashboard()
  ...
  for _, a in pairs(state.agents) do
    if a.role == "miner" then
      miners = miners + 1
    elseif a.role == "foreman" then
      foremen = foremen + 1
    elseif a.role == "gps" then
      gpshosts = gpshosts + 1
    end
  end

  monitor.setCursorPos(1, y)
  monitor.write("Controller ID: " .. tostring(os.getComputerID()))
  y = y + 1

  monitor.setCursorPos(1, y)
  monitor.write("Modem: " .. tostring(modemSide or "missing"))
  y = y + 1

  monitor.setCursorPos(1, y)
  monitor.write("Miners: " .. miners .. "  Foremen: " .. foremen .. "  GPS: " .. gpshosts)
  y = y + 1

  monitor.setCursorPos(1, y)
  monitor.write("Teams: " .. tostring(#state.teams))
  y = y + 1

  monitor.setCursorPos(1, y)
  monitor.write("Active Job: " .. tostring(state.activeJobId or "none"))
  y = y + 1
  ...
  monitor.setCursorPos(1, y)
  monitor.write("Dump: " .. (dump and (dump.x .. "," .. dump.y .. "," .. dump.z) or "unset"))
  y = y + 1

  monitor.setCursorPos(1, y)
  monitor.write("Supply: " .. (supply and (supply.x .. "," .. supply.y .. "," .. supply.z) or "unset"))
  y = y + 2
...
local function promptNumber(label, default)
  term.write(label .. (default and (" [" .. tostring(default) .. "]") or "") .. ": ")
  local s = read()
...
local function configureStorage()
  ...
  if c == "1" then
    state.storage.dump = promptCoord("dump chest")
    log("Dump chest updated.")
    saveState()
  elseif c == "2" then
    state.storage.supply = promptCoord("fuel/torches/filler chest")
    log("Supply chest updated.")
    saveState()
  end
end
...
local function configureGPS()
  ...
  print("This controller runs shell.run(\"gps\", \"host\", x, y, z) in parallel.")
  print("")
...
local function activeMiners()
  ...
  for _, a in pairs(state.agents) do
    if a.role == "miner" and os.epoch("utc") - (a.lastSeen or 0) < 45000 then
      table.insert(t, a)
    end
  end
...
local function activeForemen()
  ...
  for _, a in pairs(state.agents) do
    if a.role == "foreman" and os.epoch("utc") - (a.lastSeen or 0) < 45000 then
      table.insert(t, a)
    end
  end
...
local function computeJobBounds(job)
  ...
  if job.shape == "rect" then
    set(job.a.x, job.a.y, job.a.z)
    set(job.b.x, job.b.y, job.b.z)
...
local function compactJob(job)
  return {
    ...
    shape = job.shape,
    status = job.status,
    origin = compactCoord(job.origin),
    a = compactCoord(job.a),
    b = compactCoord(job.b),
    radius = job.radius,
    height = job.height,
    width = job.width,
...
local function compactSector(sector)
  return {
    ...
    count = sector.count,
    bounds = {
      minX = sector.bounds.minX,
      maxX = sector.bounds.maxX,
      minY = sector.bounds.minY,
      maxY = sector.bounds.maxY,
      minZ = sector.bounds.minZ,
      maxZ = sector.bounds.maxZ
    }
  }
end
...
local function compactStorage()
  return {
    dump = compactCoord(state.storage.dump),
    supply = compactCoord(state.storage.supply)
  }
end
  ...
  for i, team in ipairs(state.teams) do
    ...
    if team.foremanNet then
      safeSend(
        ...
        PROTOCOL,
        "ASSIGN_FOREMAN_" .. tostring(team.id)
      )
    end
    ...
    for _, team in ipairs(state.teams) do
      if team.id == first then
        safeSend(
          ...
          PROTOCOL,
          "DEPLOY_NOW_MINER_" .. tostring(team.id)
        )
        ...
        if team.foremanNet then
          safeSend(
            ...
            PROTOCOL,
            "DEPLOY_NOW_FOREMAN_" .. tostring(team.id)
          )
        end
...
local function pauseJob()
  for _, a in pairs(state.agents) do
    safeSend(a.netId, { type = "PAUSE_JOB" }, PROTOCOL, "PAUSE_" .. tostring(a.id))
  end
...
local function resumeJob()
  for _, a in pairs(state.agents) do
    safeSend(a.netId, { type = "RESUME_JOB" }, PROTOCOL, "RESUME_" .. tostring(a.id))
  end
...
local function cancelJob()
  for _, a in pairs(state.agents) do
    safeSend(a.netId, { type = "CANCEL_JOB" }, PROTOCOL, "CANCEL_" .. tostring(a.id))
  end
...
local function gpsDiagnostics()
    ...
    if a.role == "gps" then
      print(
        " - "
          ...
          .. tostring(a.z)
          .. " last "
          .. tostring(math.floor((os.epoch("utc") - (a.lastSeen or 0)) / 1000))
          .. "s ago"
      )
    end
  end
  ...
  print("")
  print("Controller self gps.locate() is not required to validate the constellation.")
  print("Press any key.")
  os.pullEvent("key")
end
...
local function menuLoop()
  while running do
    ...
    if k == keys.up then
      selectedMenu = math.max(1, selectedMenu - 1)
    elseif k == keys.down then
      selectedMenu = math.min(#menu, selectedMenu + 1)
    elseif k == keys.enter then
      ...
      if not ok then
        print("MENU ERROR:")
        print(tostring(err))
        rawLog("MENU ERROR: " .. tostring(err))
        saveState()
        sleep(4)
      end
    end
  ...
  while running do
    ...
    if type(msg) == "table" then
      ...
      if msg.type == "REGISTER" then
        local id = tostring(msg.role or "agent") .. "-" .. tostring(sender)
      ...
      elseif msg.type == "REQUEST_ASSIGNMENT" then
        if state.activeJobId then
          log("Assignment recovery requested by " .. tostring(sender))
          assignActiveJob()
        end
      ...
      elseif msg.type == "ERROR" then
        log("ERROR from " .. tostring(sender) .. ": " .. tostring(msg.message))
      end
    end
  end
end
