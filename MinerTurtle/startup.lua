...
local function header()
  ...
  if state.home then print("Home: " .. state.home.x .. "," .. state.home.y .. "," .. state.home.z .. " facing " .. tostring(state.homeFacing)) end
  print("")
...
local function isInventoryName(name)
  ...
  return name:find("chest",1,true) or name:find("barrel",1,true) or name:find("shulker",1,true)
end
...
local function inspectIsProtected(inspector)
  ...
  for _, n in ipairs(protectedNeedles) do
    if name:find(n, 1, true) then return true, data end
  end
  return false, data
end
...
local function turnLeft()
  ...
  save()
  sendTo(state.foremanNet, {type="MINER_TURNED_LEFT", teamId=state.teamId, pos=clonePos(state.pos), facing=state.facing})
end
...
local function turnRight()
  ...
  save()
  sendTo(state.foremanNet, {type="MINER_TURNED_RIGHT", teamId=state.teamId, pos=clonePos(state.pos), facing=state.facing})
end

local function turnTo(dir)
  if not dir then return end
  if not state.facing then state.facing = dir; save(); return end
  local guard = 0
...
local function requestForemanMove()
  if not state.foremanNet then return true end
  sendTo(state.foremanNet, {type="FOREMAN_MOVE_REQUEST", teamId=state.teamId, pos=clonePos(state.pos), facing=state.facing})
  local timer = os.startTimer(4)
  while true do
    local ev, a, b, c = os.pullEvent()
    if ev == "rednet_message" then
      local sender, msg, proto = a, b, c
      if proto == PROTOCOL and sender == state.foremanNet and type(msg) == "table" and msg.type == "FOREMAN_MOVED" then
...
local function moveForwardRaw()
  ...
  for _=1,3 do
    if turtle.forward() then
      updateForward(); save()
      sendTo(state.foremanNet, {type="MINER_MOVED_FORWARD", teamId=state.teamId, pos=clonePos(state.pos), facing=state.facing})
      return true
    end
...
local function moveUpRaw()
  ...
  for _=1,3 do
    if turtle.up() then
      ...
      sendTo(state.foremanNet, {type="MINER_MOVED_UP", teamId=state.teamId, pos=clonePos(state.pos), facing=state.facing})
      return true
    end
...
local function moveDownRaw()
  ...
  for _=1,3 do
    if turtle.down() then
      state.pos.y = state.pos.y - 1; save()
      sendTo(state.foremanNet, {type="MINER_MOVED_DOWN", teamId=state.teamId, pos=clonePos(state.pos), facing=state.facing})
      return true
    end
...
local function goX(x)
  if state.pos.x < x then
    turnTo("east")
    while state.pos.x < x do if not safeForward() then return false end end
...
local function goZ(z)
  if state.pos.z < z then
    turnTo("south")
    while state.pos.z < z do if not safeForward() then return false end end
...
local function gotoXYZ(p)
  if not state.pos or not p then return false end
  -- Preserve existing safe travel style: vertical first to the higher Y, then X/Z, then exact Y.
...
local function splinePoint(job,t)
  ...
  local u = 1-t
  return {x=u*u*ax+2*u*t*cx+t*t*bx, y=u*u*ay+2*u*t*cy+t*t*by, z=u*u*az+2*u*t*cz+t*t*bz}
end
...
local function jobMaxY(job)
  ...
  if job.shape == "dome" then return job.origin.y + job.radius end
  if job.shape == "rect" then return math.max(job.a.y, job.b.y) end
  return nil
end
...
local function inside(job,x,y,z)
  ...
  elseif job.shape == "cylinder" then
    if y < job.origin.y or y > job.origin.y + jobLayers(job) - 1 then return false end
...
local function columnFromIndex(idx)
  ...
  if batch >= batchCount() then return nil end
  ...
  if endY > startY then travelY = startY + 1 end
  return {x=b.minX+xOff, z=b.minZ+zOff, startY=startY, endY=endY, travelY=travelY, batch=batch}
end

local function columnHasWork(c)
  for y=c.startY,c.endY do
    if inside(state.job, c.x, y, c.z) then return true end
  end
  return false
end
...
local function nextMiningColumn()
  ...
  while state.progress < total do
    ...
    if c and columnHasWork(c) then save(); return c end
  end
  return nil
end

local function placeTorchIfNeeded(c)
  if not state.job or not c then return end
  ...
  if c.startY == floorY and c.travelY == floorY and ((math.abs(c.x) + math.abs(c.z)) % spacing == 0) then
    turtle.select(16)
...
local function fillOvercutsNearColumn(c)
  ...
  if c.travelY > (state.sector.fullBounds and state.sector.fullBounds.minY or state.sector.bounds.minY) then
    ...
    if not belowInside and not ok and turtle.getItemCount(2) > 0 then
      turtle.select(2); turtle.placeDown(); turtle.select(1)
    end
  end
end

local function mineColumn(c)
  -- Do not rotate for block-facing. The movement planner leaves us facing the next path direction.
  -- The turtle's occupied block is the current layer. Movement into this column already cleared it if needed.
  if c.startY < c.travelY and inside(state.job, c.x, c.travelY-1, c.z) then digDownIfSafe() end
  if c.endY > c.travelY and inside(state.job, c.x, c.travelY+1, c.z) then digUpIfSafe() end
...
local function refillSlotFromUp(slot)
  ...
  if need > 0 then turtle.suckUp(need) end
end

local function refillFillerFromBelowOrInventory()
  -- Slot 2 is filler. Prefer merging matching filler from inventory, then pull from deposit chest below.
  mergeFiller()
  turtle.select(2)
  if turtle.getItemCount(2) < 64 then turtle.suckDown(64 - turtle.getItemCount(2)) end
  mergeFiller()
  turtle.select(1)
end

local function goHomeAndService(reason, returnAfter)
  if not state.home then return false end
  local returnPos = nil
  if returnAfter and state.pos then returnPos = {x=state.pos.x,y=state.pos.y,z=state.pos.z,f=state.facing} end
  status(reason or "GOING_HOME")
  sendTo(state.foremanNet, {type="SERVICE_RETURN", teamId=state.teamId, pos=clonePos(state.pos), home=clonePos(state.home)})
  if not gotoXYZ(state.home) then status("HOME_PATH_FAILED"); return false end
  turnTo(state.homeFacing or state.facing or "north")

  status("HOME_SERVICE")
  refillFillerFromBelowOrInventory()
  for i=3,15 do turtle.select(i); turtle.dropDown() end
  refillSlotFromUp(1)
  refillSlotFromUp(16)
  turtle.select(1)
  refuelIfNeeded()
  ...
  if returnPos then
    ...
    if gotoXYZ(returnPos) then turnTo(returnPos.f); status("MINING"); return true end
...
local function ensureHomeSetup()
  calibrate()
  if state.home and state.homeFacing then return true end
  while running do
...
local function waitForDeploy()
  ...
  while state.deployHold and running do sleep(1) end
end
...
local function mineLoop()
  ...
  if state.sector.bounds.maxY > firstY then firstY = firstY + 1 end

  status("MOVE_TO_ORIGIN")
  gotoXYZ({x=origin.x, y=firstY, z=origin.z})
  ...
  while running and not state.complete do
    if state.forceHome then
      ...
      state.forceHome = false; state.resumeAfterHome = false; save()
    ...
    if state.paused then
      status("PAUSED")
      repeat sleep(1) until not state.paused or not running or state.forceHome
      if state.forceHome then -- let loop handle it immediately
      else status("MINING") end
    end

    mergeFiller()
    if needsService() then goHomeAndService("SERVICE_RETURN", true) end
    ...
    local ok = gotoXYZ({x=c.x, y=c.travelY, z=c.z})
    if not ok then
      send({type="ERROR", teamId=state.teamId, message="Could not path to mining column; returning home."})
  ...
  while running do
    ...
    if type(msg) == "table" then
      ...
      elseif msg.type == "GO_HOME" then
        ...
        save(); status("GO_HOME_REQUESTED")
      elseif msg.type == "CANCEL_JOB" then
        state.job=nil; state.sector=nil; state.complete=false; state.progress=0; state.forceHome=true; state.resumeAfterHome=false
        save(); status("CANCELLED_GOING_HOME")
      elseif msg.type == "ROLL_CALL" then
        send({type="ROLL_CALL_RESPONSE", role="miner", status=state.status, x=state.pos and state.pos.x, y=state.pos and state.pos.y, z=state.pos and state.pos.z})
      end
    end
  end
end
...
local function heartbeatLoop()
  while running do
    send({type="REGISTER", role="miner", label=state.label, status=state.status, x=state.pos and state.pos.x, y=state.pos and state.pos.y, z=state.pos and state.pos.z})
    send({type="HEARTBEAT", role="miner", label=state.label, status=state.status, x=state.pos and state.pos.x, y=state.pos and state.pos.y, z=state.pos and state.pos.z, facing=state.facing})
    sleep(10)
  end
end
